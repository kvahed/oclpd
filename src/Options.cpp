...
bool  Options::doubleOptStorage     ()                                          {
	...
	/* init new storage */
	for (size_t i = max_options ; i < 2*max_options ; ++i){
		options    [i] = NULL;
		optiontype [i] =  0 ;
		optionindex[i] = -1 ;
	}
...
bool  Options::doubleCharStorage    ()                                          {
	...
	/* init new storage */
	for (size_t i = max_char_options ; i < 2*max_char_options ; ++i){
		optionchars [i] = '0';
		optchartype [i] =  0 ;
		optcharindex[i] = -1 ;
	}
...
bool  Options::doubleUsageStorage   ()                                          {
	...
	for (size_t i = max_usage_lines ; i < 2*max_usage_lines ; ++i)
		usage[i] = NULL;
...
void  Options::processCommandArgs   ()                                          {
	...
	for (size_t i = 1 ; i < argc ; ++i) {                                              /* ignore first argv */
...
int   Options::matchOpt             (char *opt) {
	...
	for (size_t i = 0; i < option_counter; ++i) {
...
bool  Options::matchChar            (char c) {
	...
	for (size_t i = 0 ; i < optchar_counter ; ++i) {
...
bool  Options::valueStoreOK         () {
	...
	if (!set) {
		if (g_value_counter > 0) {
			...
			for (size_t i = 0; i < g_value_counter; ++i)
				values[i] = NULL;
			set    = true;
		}
	}
...
char* Options::getArgv (size_t index) {
	if (index < new_argc)
 ...
 */
char* Options::readFile (const std::string& fname) {

        int length;
        char *buffer;
        ifstream is;
        is.open (fname.c_str(), ifstream::in);
...
void  Options::processLine          (char *theline, int length) {
	...
	if (*cursor   == delimiter || *(cursor+length-1) == delimiter)
		...
		for (size_t i = 1 ; i < length-1 && !found ; ++i){                             /* delimiter */
			if (*cursor == delimiter){
...
void Options::printUsage            () {
	if (once) {
		once = false ;
		cout << endl ;
		for (size_t i = 0 ; i < usage_lines ; ++i)
			cout << usage[i] << endl ;	
		cout << endl ;
	}
}
